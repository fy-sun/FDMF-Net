...
class SelfAttentionBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.mlp = Mlp_DWConv(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, y, attn_mask=None):
        ...
        if self.use_flash and x.is_cuda and attn_mask is None:
            out = F.scaled_dot_product_attention(
                q, k, v,
                dropout_p=self.attn_drop.p if self.training else 0.0,
                is_causal=False)  # Flash 内核
        else:
            attn = (q @ k.transpose(-2, -1)) * self.scale
            attn = attn.softmax(dim=-1)
            attn = self.attn_drop(attn)
...
class CrossAttentionBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.extra_norm1 = norm_layer(dim)
    ...
    def forward(self, x, y, H, W):
        x = x + self.drop_path(self.attn(self.norm1(x), self.extra_norm1(y)))
        x = x + self.drop_path(self.mlp(self.norm2(x), H, W))
